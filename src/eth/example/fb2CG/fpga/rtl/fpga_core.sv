...
// QSFP28
localparam MAC_CNT = MAC_DATA_W > 64 ? 2 : 8;
...
if (MAC_CNT == 2) begin
    assign led_g[0] = qsfp_rx_status[0];
    assign led_g[1] = 1'b0;
    assign led_g[2] = 1'b0;
    assign led_g[3] = 1'b0;
    assign led_g[4] = qsfp_rx_status[1];
    assign led_g[5] = 1'b0;
    assign led_g[6] = 1'b0;
    assign led_g[7] = 1'b0;
end else begin
...
taxi_axis_if #(.DATA_W(16), .KEEP_W(1), .KEEP_EN(0), .LAST_EN(0), .USER_EN(1), .USER_W(1), .ID_EN(1), .ID_W(8)) axis_eth_stat[2]();
...
for (genvar n = 0; n < 2; n = n + 1) begin : gt_quad
    ...
    if (MAC_DATA_W == 512) begin: mac
        ...
        mac_inst (
             ...
             * MAC clocks
             */
            .rx_clk(qsfp_rx_clk[n]),
            .rx_rst_in(1'b0),
            .rx_rst_out(qsfp_rx_rst[n]),
            .tx_clk(qsfp_tx_clk[n]),
            .tx_rst_in(1'b0),
            .tx_rst_out(qsfp_tx_rst[n]),

            /*
             * Transmit interface (AXI stream)
             */
            .s_axis_tx(axis_qsfp_tx[n]),
            .m_axis_tx_cpl(axis_qsfp_tx_cpl[n]),

            /*
             * Receive interface (AXI stream)
             */
            .m_axis_rx(axis_qsfp_rx[n]),
