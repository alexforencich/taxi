...
always_ff @(posedge clk) begin
    ...
    if (GBX_IF_EN && tx_gbx_req_stall) begin
        ...
        if (output_start_packet_reg) begin
            start_packet_reg <= 1'b1;
            if (PTP_TS_EN) begin
                m_axis_tx_cpl_ts_reg <= ptp_ts + (PTP_TS_COR_EN ? PTP_TS_W'(ptp_ts_cor_val) + (GBX_IF_EN && gbx_en_reg && phase_reg == 1 ? PTP_TS_W'(ts_inc_reg >> 5) : '0) : '0);
            end
